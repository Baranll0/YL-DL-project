...
Bu tabloya karşılık gelen tüm grafikler `results/` klasörü altındadır ve bu README kök dizinde olduğu için yollar doğrudan `results/...` şeklindedir.
...
3. **Gelişmiş HOG + renk + LBP ailesi (`hog_enh_*`)**
   ...
   - `hog_enh_equalize_scaler` → ≈ **%56.7** (eşitleme + scaler ile HOG+Scaler’a yakın bir performанс).
...
4. **Genel sıralama ve kazanç**
   ...
   - Ham pikselden en iyi HOG+renk+LBP конfigurationuna geçerken yaklaşık **+22 puan** doğruluk kazanılmıştır.
...
- yüksek boyutlu ham пикsel узayında **curse of dimensionality** nedeniyle uzaklık hesabının anlamsızlaştığını,
